...
parser.add_argument('--dir', default="out_brs_digit")
...
for it in range(1000000):
    ...
    while(1):
        X_mb_temp, Y_mb_temp = mnist.train.next_batch(mb_size*20)
        X_mb = []
        count = 0
        for idx  in range(mb_size*20):
            if Y_mb_temp[idx,digit] == 1:
                X_mb.append(X_mb_temp[idx,:])
                # X_mb[count,] = X_mb_temp[idx,]
                count = count + 1
            if(count == mb_size):
                break
        if(count==mb_size):
