...
hidden_dim = 10
search_num = 20 
top_num = 20 # only top performance is used
alpha = 1
v = 0.02
...
def discriminator(x):
    ...
    # D_logit = tf.matmul(tf.to_float(x), D_W1) + D_b1
    # D_prob = tf.nn.sigmoid(D_logit)
...
G_loss = tf.reduce_mean(tf.log(tf.clip_by_value(D_fake, 1e-16, 1.0)))
    ...
    for i in range(1):
        ...
        for m in range(search_num):
            ...
            if m == 0: 
                ...
                for t in range(len(G.theta_G)):
                    update[t] += reward_list[m] / reg * delta_list[m][t]
