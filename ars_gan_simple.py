...
D_loss = -tf.reduce_mean(tf.log(tf.clip_by_value(D_real, 1e-3, 1.0)) + tf.log(tf.clip_by_value(1. - D_fake, 1e-3, 1.0)))
G_loss = -tf.reduce_mean(tf.log(tf.clip_by_value(D_fake, 1e-3, 1.0)))

S_fake, S_logit_fake = discriminator(S_sample)

S_loss = -tf.reduce_mean(tf.log(tf.clip_by_value(S_fake, 1e-3, 1.0)))
# Alternative losses:
# -------------------
...
for it in range(1000000):
    ...
    for m in range(search_num):
        delta = []
        for t in range(len(G.theta_G)):
            delta.append(np.random.normal(loc=0.0, scale=1./100,
                                        size=G.size[t]))
        for t in range(len(G.theta_G)):
